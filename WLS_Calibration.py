...
###==================== 基于FFT峰值进行IQ校准 ===================
def run_fft_peak_calibration(iq_virtual_data: np.ndarray):
    """
    [最终极简版 - 仅执行 FFT 峰值校准]

    此函数接收 reorder_frame_TDMMIMO 的输出，自动查找峰值 bin，
    并提取复数增益，最后返回 apply_channel_calibration 所需的 (2,2) 矩阵。

    参数:
    iq_virtual_data (np.ndarray): 形状 (4, N_obs, N_samples)
                                 (例如 (4, 16, 256))
                                 这是 reorder_frame_TDMMIMO 的输出。

    返回:
    (np.ndarray | None, np.ndarray | None, int | None):
        - alpha_matrix (2, 2): 幅度校准矩阵
        - phi_matrix (2, 2):   相位校准矩阵 (rad)
        - detected_bin (int):  自动检测到的 target_range_bin
    ...
    if iq_virtual_data.ndim != 3 or iq_virtual_data.shape[0] != 4:
        print(f"错误: 输入数据维度必须是 (4, N_obs, N_samples), 实际为 {iq_virtual_data.shape}")
    ...
    print(f"输入数据: {M_virtual} 虚通道, {N_obs} Chirps, {N_samples} 采样点")
    ...
    avg_range_profile[0] = 0  # 忽略直流分量

    final_target_bin = int(np.argmax(avg_range_profile))
    print(f"自动检测完成：峰值位于 Range Bin = {final_target_bin}")

    # --- 4. 阶段 3: 提取复数增益 ---
    # 提取所有通道在峰值 bin 上的复数值
    # shape = (4, 16)
    peak_complex_values = range_fft_results[:, :, final_target_bin]
    try:
        g_virtual = np.mean(peak_complex_values, axis=1)
        print("复数增益提取并平均完毕。")
    except Exception as e:
        print(f"错误: 相干平均失败: {e}")
        return (None, None, final_target_bin)

    # --- 5. 阶段 4: 构建 (2,2) 矩阵 ---
    try:
        # 将 (4,) 向量按 C 语言顺序重塑为 (K_TX, L_RX) -> (2, 2)
        complex_gain_matrix = g_virtual.reshape((K_TX, L_RX))

        # 分离幅度和相位
        amplitude_matrix = np.abs(complex_gain_matrix)
        phase_matrix = np.angle(complex_gain_matrix)
        ...
        print(f"错误: 无法将 (4,) 向量重塑为 ({K_TX}, {L_RX}): {e}")
        return (None, None, final_target_bin)
